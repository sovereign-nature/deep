<script setup lang="ts">
  ...
  $: pageDescription = nftData?.description || '';
  $: name = nftData!.name;
  ...
  $: pageImagePath = `/og?title=${encodeURIComponent(
    ...
    image
  )}&source=${encodeURIComponent(source)}&${ecoLinked ? '&ecoLinked' : ''}`;
  ...
  >
    ...
    <div class="lg:col-span-8 xl:col-span-8">
      <div>
        ...
        >
          {#if ecoLinked}
            <span class="text-primary-300">{$LL.assets.ecoLinked()}</span>
          {/if}
                  ...
                  {#each Object.keys(properties.traces_recorded) as property}
                    {#if properties.traces_recorded[property] > 0}
                      ...
                      >
                        From {property}:
                        <span
                          class="text-primary-400 dark:text-primary-300 ms-1"

<script lang="ts">
  ...
  function setTabFromSelect(e: Selected<unknown> | undefined) {
    const selected: number =
      e && typeof e.value === 'number' ? e.value : $startIndex;
...
<div class="flex flex-col w-full">
  <Select.Root
    ...
    selected={{ value: $activeTab, label: $tabs[$activeTab].label }}
  >
    <Select.Label class="text-center font-normal	">
      <span class="dark:text-gray-400 text-sm"> {$titleFull}</span>
    </Select.Label>
    <Select.Trigger>
    ...
    <Select.Content>
      <Select.Group class="flex flex-col">
        {#each $tabs as item, index}
          <Select.Item value={index} label={item.label}>
            <ItemCard {item} active={$activeTab === index} />
          </Select.Item>
        {/each}
      </Select.Group>
    </Select.Content>
